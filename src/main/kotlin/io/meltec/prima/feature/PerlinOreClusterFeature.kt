...
/**
 * Configure perlin ore generation. The width determines the X/Z bounding box size, while
 * the height determines the Y bounding box. The threshold should be between 0.0 and 1.0 and determine
 * how high the perlin noise has to be for a given tile to be a solid block; values between 0.1 and 0.4
 * are recommended.
 */
data class PerlinOreClusterFeatureConfig(val state: BlockState, val width: Int, val height: Int, val threshold: Double) : FeatureConfig {
    companion object {
        val CODEC = RecordCodecBuilder.create<PerlinOreClusterFeatureConfig> {
            it.group(
 ...
 */
object PerlinOreClusterFeature : Feature<PerlinOreClusterFeatureConfig>(PerlinOreClusterFeatureConfig.CODEC) {
    private const val DOWNSCALE_FACTOR = 32.0

    override fun generate(world: StructureWorldAccess, chunkGenerator: ChunkGenerator, random: Random, pos: BlockPos, config: PerlinOreClusterFeatureConfig): Boolean {
        ...
        for (x in startPosition.x .. endPosition.x) {
            for (y in startPosition.y .. endPosition.y) {
                for (z in startPosition.z .. endPosition.z) {
                    ...
                    val distance = pos.getManhattanDistance(position).toDouble() / config.width.coerceAtLeast(config.height)
                    if (random.nextDouble() < distance) continue

                    val noise = octaveSampler.sample(
